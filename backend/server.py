...
from backend.trending import top_keywords   # NEW
import yfinance as yf

app = FastAPI(title="Stock Backend", version="0.6")
...
# ---------------- Helper: normalize sentiment ----------------
def _sentiment_label(raw: Any) -> str:
    """
    Normalize whatever summarize(...) returns into a plain sentiment label string.

    Handles:
    - dict: {"label": "Positive", "score": 0.8}
    - dict with "summary" or "text"
    - plain string: "Positive" / "Neutral" / etc.
    - None
    """
...
def reddit(
    ...
    q = (symbol or ticker or "").upper()
    return fetch_posts(q, days=days, limit=limit)
...
def sentiment(ticker: str, days: int = 14, limit: int = 50) -> Dict[str, Any]:
    ...
    raw_summary = summarize(series)
    summary_text = _sentiment_label(raw_summary)
...
def trending(req: TrendingRequest):
    """
    Returns trending keywords from Reddit posts and/or client-provided headlines.
    """
    items: List[Dict[str, Any]] = []

    # Add client headlines
    for h in req.headlines:
        ts = h.publishedAt or datetime.now(timezone.utc)
        items.append({"text": h.title, "ts": ts})

    # Add Reddit posts if requested
    if req.include_reddit:
        if req.reddit_tickers:
            for t in req.reddit_tickers:
                try:
                    posts = fetch_posts(t, days=req.days, limit=100)
                    for p in posts:
                        items.append(
                            {
                                "text": p.get("title", ""),
                                "ts": p.get("created_at_dt"),
                            }
                        )
                except Exception as e:
                    print(f"[WARN] Reddit fetch for {t} failed: {e}")
        else:
            try:
                posts = fetch_posts("market", days=req.days, limit=200)
                for p in posts:
                    items.append(
                        {
                            "text": p.get("title", ""),
                ...
                print(f"[WARN] Reddit fetch (general market) failed: {e}")

    result = top_keywords(items, window_days=req.days, limit=req.limit)
    return result
